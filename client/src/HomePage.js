...
export default function HomePage(props) {
    ...
    const getCurrentModal = () => {
        ...
        if (modalType === 'resources') {
            ...
            modal = <NewLogin switchToLanguage={props.switchToLanguage} showModal={showModal} hideModal={handleHideModal}/>
        } else if (modalType === 'request') {
            modal = <RequestHelp setSwithToLanguage={props.setSwithToLanguage} switchToLanguage={props.switchToLanguage}
                                 requestHelpMode={mode} showModal={showModal} hideModal={handleHideModal} 
    ...
    return ([
        <div key="1" className="App" style={{height: '100%'}}>
            ...
            <NavBar setSwithToLanguage={props.setSwithToLanguage} switchToLanguage={props.switchToLanguage} pageLoaded={pageLoaded} isLoggedIn={loggedIn} 
                first_name={currentUser.first_name} handleShowModal={showModalType}/>
            <Jumbotron fluid id="jumbo">
                <Container id="jumboContainer">
                    <Row>
                        <Col md={6} id="jumbo-text">
                            <h1 id="home-heading">{translatedStrings[language].HomePage_Title}</h1>
                            <p id="home-subheading">{translatedStrings[language].HomePage_Subtitle}</p>
                            {supportButton(props.currentAssoc, updateRequestHelpMode, language)}{' '}
                            {volunteerButton(loggedIn, language)}<br/>
                            <Button variant="link" id="underlined-link" onClick={() => showModalType('resources')}>
                                <u>{translatedStrings[language].HomePage_ViewResources}</u>
                            </Button>
                        </Col>
                        <Col md={6} id="community-bulletin">
                            <p id='location-text'>{translatedStrings[language].HomePage_VolunteersAround} 
                                <button id="change-location" onClick={() => showModalType('location')}> {props.locality}</button>
                            </p>
                            <p id="volunteer-info">{translatedStrings[language].HomePage_ClickInfo}</p>
                            <CommunityBulletin switchToLanguage={props.switchToLanguage} googleAPI={props.googleAPI} volunteers={volunteers} resources={props.resources} mobile={false}
            ...
            <Container id={loggedIn ? "location-container-logged" : "location-container"}>
                <CitySupport switchToLanguage={props.switchToLanguage} currentAssoc={props.currentAssoc}/>
            </Container>
            <Container id="jumboContainer" className={"mobile-bulletin-container"}>
                <Col xs={12} id="mobile-bulletin">
                    ...
                    <p id="volunteer-info" style={{color: 'black', float: 'left', width: '100%', fontSize: "3vw"}}>
                        {translatedStrings[language].HomePage_ClickInfo}
                    </p>
                    <CommunityBulletin switchToLanguage={props.switchToLanguage} googleAPI={props.googleAPI} volunteers={volunteers} resources={props.resources} mobile={true}
                                       handleShowRequestHelp={(modalInfo) => updateRequestHelpMode('bulletin', modalInfo)}/>
                    {cantFindLink(props.currentAssoc, updateRequestHelpMode)}
                </Col>
            </Container>
