...
export default function OrganiationPortal(props) {
	...
	if (beaconView) {
		...
		return <OrgLogin setSwithToLanguage={props.setSwithToLanguage} switchToLanguage={props.switchToLanguage} login={login} setShowLogin={setShowLogin} orgReset={props.location.orgReset}/>
	} else if (!pageLoaded) {
		return <></>;
	}
	return ([
		<div className="App" key="1">
			<NavBar setSwithToLanguage={props.setSwithToLanguage} switchToLanguage={props.switchToLanguage}
					pageLoaded={pageLoaded} isLoggedIn={true} totalVolunteers={volunteers.length} 
