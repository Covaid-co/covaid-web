...
export default function RequestColumn(props) {
  ...
  const requestCards = () => {
    if (props.requests.length > 0) {
        return props.requests.map((request, i) => {
            return <RequestCard request={request} requestStatus={props.requestStatus} color={color}/>
        });
    } else {
        return (
            <RequestCard empty={true} requestStatus={props.requestStatus}/>
        );
    }
  }
  ...
  return (
      <>
        <p id="regular-text" style={{ fontSize: 16, color: '#4F4F4F', fontWeight: 600 }}>
            {title}
        </p>
