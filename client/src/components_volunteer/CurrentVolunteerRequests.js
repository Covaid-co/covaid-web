...
export default function CurrentVolunteerRequests(props) {
      ...
      <>
        ...
        >
          <strong style={{ fontSize: 18 }}>
            You have no pending/active requests at the moment.
          </strong>
        </p>
      </>
    );
  ...
  const handleRequestClick = (request, mode) => {
    ...
    setModalOpen(true);
    setModalMode(mode);
    let volunteerSpecific = request.status.volunteers.filter(function (
      volunteer
    ) {
        ...
        {acceptedRequests.map((request, i) => {
          return (
            <ListGroup.Item
              ...
              key={i}
              onClick={() => {
                handleRequestClick(request, volunteer_status.IN_PROGRESS);
              }}
            >
              <div>
                ...
                <Badge style={{ float: "right" }} className="in-progress-task">
                  In Progress
                </Badge>
              </div>
              <div>
              ...
              >
                <p style={{ float: "left", marginBottom: 0 }} id="regular-text">
                  Date created: {request.request_info.date}
                </p>
              </div>
            </ListGroup.Item>
          );
        ...
        {pendingRequests.map((request, i) => {
          ...
          return (
            <ListGroup.Item
              ...
              disabled={disabled}
              onClick={() => {
                handleRequestClick(request, volunteer_status.PENDING);
              }}
            >
              <div>
                <h5 id="volunteer-name">
