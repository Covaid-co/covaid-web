...
export default function RequestDetails(props) {
  ...
  const updateAdminList = () => {
    if (props.association.admins) {
      ...
      if (
        adminNames.includes(props.currRequest.admin_info.assignee) === false
      ) {
  ...
  const deleteRequest = () => {
    ...
    fetch("/api/request/updateRequestDetails", {
      ...
      .then((response) => response.json())
      .then((request) => {
        const newAllRequests = props.allRequests.filter(
  ...
  const adminTracker = () => {
    if (
      ...
      (Object.keys(props.admin).length !== 0 ||
        props.admin.constructor !== Object)
    ) {
      return (
        <Row>
  ...
  const volunteerComments = () => {
    ...
    if (
      props.mode === current_tab.COMPLETED &&
      found &&
      ...
      return (
        <>
          <h5 id="regular-text-bold" style={{ marginBottom: 0, marginTop: 16 }}>
            Volunteer's completion remarks:
          </h5>
  ...
  const requestStatus = () => {
    if (props.mode === current_tab.MATCHED) {
      const volunteers = props.currRequest.status.volunteers;
      const in_progress = volunteers.find(
        (vol) => vol.current_status === volunteer_status.IN_PROGRESS
      );
      const pending = volunteers.find(
        (vol) => vol.current_status === volunteer_status.PENDING
      );
      if (in_progress) {
        return (
    ...
    if (props.association._id === "5e843ab29ad8d24834c8edbf") {
      ...
      return (
        <p id="regular-text-nomargin">
          Payment: {paymentOptions[props.currRequest.request_info.payment]}
        </p>
      );
    }
  };
  ...
  const contactInfo = () => {
    if (
      ...
      return (
        <>
          <p id="regular-text-nomargin" style={{ color: "#EF6315" }}>
            <b>Share contact directly with volunteer (i.e. from Notes)</b>
          </p>
        </>
      );
    }
  ...
  return (
    <>
      <Modal
        show={props.requestDetailsModal}
        onHide={() => {
          props.setRequestDetailsModal(false);
          setNotes();
      ...
      >
        <Modal.Body>
          <h5 id="regular-text-bold" style={{ marginTop: 0, marginBottom: 5 }}>
            Who's tracking this request:
          </h5>
          {adminTracker()}
          <h5 id="regular-text-bold" style={{ marginTop: 13, marginBottom: 5 }}>
            Your Notes:
          </h5>
          <Form>
          ...
          <Modal.Title id="small-header" style={{ marginTop: 20 }}>
            Request Details {modeString()} {requestStatus()}
          </Modal.Title>
          ...
          <div>
            <p id="name-details" style={{ display: "inline" }}>
          ...
          <p id="regular-text-nomargin" style={{ marginTop: 14 }}>
            Languages: {props.currRequest.personal_info.languages.join(", ")}
          </p>
          {paymentText()}
      ...
      <Modal
        show={showRequestMapModal}
        onHide={() => {
          setShowRequestMapModal(false);
          props.setRequestDetailsModal(true);
        }}
        style={{ marginTop: 10, paddingBottom: 50 }}
      >
        <RequestDetailMapModal
          currRequest={props.currRequest}
          mapboxAccessToken={mapboxAccessToken}
        />
      </Modal>
      ...
      >
        ...
        <Modal.Body>
          ...
          <Button
            id="large-button"
            style={{ backgroundColor: "#DB4B4B", border: "1px solid #DB4B4B" }}
      ...
      >
        ...
        <Modal.Body>
          <p id="regular-text" stlye={{ marginBottom: 5 }}>
      ...
      <BestMatches
        {...props}
        topMatchesModal={topMatchesModal}
