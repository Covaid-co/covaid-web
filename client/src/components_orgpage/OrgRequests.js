...
export default function OrgRequests(props) {
  ...
  const sortInfo = (request) => {
    ...
    return (
      <p
        id="regular-text"
        style={{ float: "right", marginBottom: 0, marginRight: 10 }}
      >
        {temp_sortType}: {formatted_time}
      </p>
    );
  };
  ...
  const displayAdmin = (request) => {
    const assigneeString = request.admin_info.assignee
    ...
    return (
      <p style={{ float: "left", marginBottom: 0 }}>
        ...
        <font
          style={
            assigneeString !== "No one assigned"
              ? { color: "#2670FF" }
              : { color: "#EF6315" }
          }
        >
  ...
  const requestStatus = (request) => {
    if (props.mode === current_tab.MATCHED) {
      ...
      if (in_progress) {
        return (
          <Badge className="in-progress-task" style={{ marginRight: 8 }}>
            In Progress
          </Badge>
        );
      } else {
        if (isInPending(request)) {
          return (
            <Badge className="pending-task" style={{ marginRight: 8 }}>
              Pending
            </Badge>
          );
        } else {
      ...
      <Col xs={8}>
        <Form>
          <Form.Group bssize="large">
            <Form.Control
              id="filter-requests"
              placeholder="Search by First/Last name, Assignee or task"
          ...
          {filteredRequests.map((request, i) => {
            return (
              ...
              >
                ...
                <div>
                  {displayBadges(request)} {requestStatus(request)}
                </div>
                <div
                  style={{
