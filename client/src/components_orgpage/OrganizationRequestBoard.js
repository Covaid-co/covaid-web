...
export default function OrganizationRequestBoard(props) {
  ...
  useEffect(() => {
    setUnmatchedRequests(
      filter_requests(props.allRequests, current_tab.UNMATCHED)
    );
    setMatchedRequests(filter_requests(props.allRequests, current_tab.MATCHED));
    setCompletedRequests(
      filter_requests(props.allRequests, current_tab.COMPLETED)
    );
    setIsLoaded(true);
  }, [props.allRequests]);
  ...
  const tabID = (tabNumber) => {
    return tabNumber === props.currTabNumber
      ? "tab-button-selected"
      : "tab-button";
  };
