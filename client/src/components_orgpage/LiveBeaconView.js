...
export default function LiveBeaconView(props) {
    ...
    useEffect(() => {
        setIsLoaded(true);
        setActiveBeacons(props.beacons.filter(function (beacon) {
            return beacon.beaconStatus === 1
        }));
        setInactiveBeacons(props.beacons.filter(function (beacon) {
            return beacon.beaconStatus === 2;
        }));
        setCompletedBeacons(props.beacons.filter(function (beacon) {
    ...
    const move = (beacon, from, to) => {
        ...
        beaconStartingStateSetter(beaconStartingState.filter(currBeacon => currBeacon._id !== beacon._id));
					...
					<Container style={{maxWidth: 1500}}>
						<Row>
							<Col style={{marginTop: 165}}>
                                ...
                                <br/>
								<Button variant="link" id="resources-link" style={{paddingLeft: 0}} onClick={()=>{window.location.reload(false)}}>
									Return to dashboard
								</Button>
							</Col>
                            <Col lg={1}></Col>
                            <Col lg={6} md={12} sm={12} style={{marginTop: 30}}>
                                <Container style={{padding: 0,  marginLeft: 0}}>
                                    <Button id={tabID(1)} onClick={() => {setCurrTab(1)}}>Active</Button>
                                    <Button id={tabID(2)} onClick={() => {setCurrTab(2)}}>Drafts</Button>
                                    <Button id={tabID(3)} onClick={() => {setCurrTab(3)}}>Complete</Button>
                                </Container>
                                <Container id="newOfferContainer" style={displayTab(1)}>
                                    <OrganizationBeacons beacons={activeBeacons} association={props.association} type={BeaconStatusEnum.active} move={move} />
                                </Container>
                                <Container id="newOfferContainer" style={displayTab(2)}>
                                    <OrganizationBeacons beacons={inactiveBeacons} association={props.association} type={BeaconStatusEnum.inactive} move={move} />
                                </Container>
                                <Container id="newOfferContainer" style={displayTab(3)}>
                                    <OrganizationBeacons beacons={completedBeacons} association={props.association} type={BeaconStatusEnum.complete} move={move} />
                                </Container>
                            </Col>				
						</Row>
					</Container>
