  ...
  return (
    ...
    >
      <Col xs={12} style={{ padding: 0, marginBottom: 10 }}>
        <p id="small-header" style={{ display: "inline" }}>
          {props.width < 600 ? "Map" : "Organization Map"}
        </p>
        <Button
          id={!volunteerMap ? "volunteer-not-selected" : "volunteer-selected"}
          onClick={() => setVolunteerMap(!volunteerMap)}
        >
          Volunteers
        </Button>
        <Button
          id={!requesterMap ? "requester-not-selected" : "requester-selected"}
