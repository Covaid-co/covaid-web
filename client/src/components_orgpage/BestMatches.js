...
export default function BestMatches(props) {
  ...
  useEffect(() => {
    ...
    var temp_checkbox = {};
    unselected.forEach(function (volunteer) {
  ...
  const handleCheckboxAction = (volunteer, event) => {
    ...
    if (!checkboxStatus[volunteer._id]) {
      ...
      volunteers = selectedVolunteers.filter(
        (selectedVolunteer) => selectedVolunteer._id !== volunteer._id
      );
    }
  ...
  const displayFilteredVolunteers = () => {
    if (filteredVolunteers.length > 0) {
      ...
      return (
        <p id="regular-text" style={{ textAlign: "center", marginTop: 20 }}>
    ...
    <>
        ...
        <Modal.Body style={{ zoom: "90%" }}>
          <Row style={{ marginTop: 0 }}>
            <Col xs="12" style={{ marginTop: 0, marginBottom: 10 }}>
              <Modal.Title>
                {bestMatchesTitle(props.currRequest, props.mode)}
                <Form style={{ float: "right" }}>
                  ...
                  >
                    <Form.Check
                      ...
                      id="custom-switch"
                      style={{ color: "#2670FF", fontSize: 14 }}
                      label={strict ? "Match on resource" : "Match on resource"}
                      checked={strict}
            ...
            <Col xs="12" id="col-scroll">
              <ListGroup variant="flush">
                {notified_volunteers.map((volunteer) =>
                  volunteerListGroup(
                    volunteer,
                    props.currRequest,
                    handleVolunteerClick,
            ...
            <Col xs="12">
              <p id="requestCall" style={{ marginTop: -20, marginBottom: 5 }}>
                &nbsp;
              </p>
              <Button
                variant="link"
                id="large-button"
                style={unselectButtonStyle(volunteerCount)}
                onClick={() => {
                  setSelectedVolunteers([]);
                  clearCheckBox();
                }}
              ...
              >
                Select {volunteerCount()} Volunteers
              </Button>
              <Button
                style={{ marginTop: 5 }}
        ...
        <Modal.Body style={{ zoom: "90%" }}>
          <Row style={{ marginTop: 0 }}>
            <Col xs="12" style={{ marginTop: 0, marginBottom: 0 }}>
              <Modal.Title>
                Notify these volunteers? ({volunteerCount()})
              </Modal.Title>
              <p id="requestCall" style={{ marginTop: -15, marginBottom: 0 }}>
                &nbsp;
              </p>
      ...
      >
        ...
        <Modal.Body>
          <p id="regular-text">
            {volunteer_count === 1
              ? volunteer_count + " volunteer has been notified"
      ...
      <VolunteerDetails
        {...props}
        volunteerDetailModal={bestMatchVolunteer}
