...
export default function AcceptedVolunteersModal(props) {
      ...
      return (
        ...
        >
          <strong style={{ fontSize: 18 }}>
            No one has accepted this beacon yet
          </strong>
        </p>
      );
    } else {
  ...
  return (
    <>
      <Modal
        size="md"
        show={props.showAcceptedVolunteers}
        onHide={() => {
          props.setShowAcceptedVolunteers(false);
          props.setOriginalModal(true);
        }}
      >
        <Modal.Header closeButton>
          <Modal.Title id="small-header">
            Beacon-Accepting Volunteers
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>{pageContent()}</Modal.Body>
      </Modal>
    </>
  );
}
