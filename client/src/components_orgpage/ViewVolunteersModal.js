...
export default function ViewVolunteersModal(props) {
    ...
    useEffect(() => {
        const ids = props.volunteers.map(volunteer => volunteer.volunteer).join(",");
        if (ids.length > 0) {
            let params = {'ids': ids};
    ...
    return (
        <Modal show={props.showVolunteers} onHide={() => props.setShowVolunteers(false)} style = {{marginTop: 5, paddingBottom: 40}}>
