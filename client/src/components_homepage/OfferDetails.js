...
export default function OfferDetails(props) {
  ...
  return (
    <Modal
      ...
      onHide={() => props.setModalOfferOpen(false)}
      style={{ marginTop: 20 }}
    >
      <Modal.Header closeButton>
        <Modal.Title id="small-header">
          {props.modalInfo.first_name}'s Offer
        </Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <>
          {props.modalInfo.pronouns === undefined ||
          props.modalInfo.pronouns === "" ? (
            ""
          ) : (
            <div>
