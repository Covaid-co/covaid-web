...
export default function BulletinOffers({
  displayedVolunteers,
  setModalInfo,
  setModalOfferOpen,
}) {
    ...
    <>
      {displayedVolunteers.map((user, i) => {
        return (
          ...
          >
            <div>
              <h5 id="volunteer-name">{user.first_name}</h5>
              <h5 id="association-name">
                {user.association_name === "" ||
                user.association_name === "Covaid"
                  ? ""
                  : user.association_name}
              </h5>
            </div>
            <p id="volunteer-location">{user.offer.neighborhoods.join(", ")}</p>
            <div>
        ...
        <div>
          <p id="regular-text" style={{ color: "black" }}>
            <strong style={{ fontSize: 18 }}>
              There seems to be no offers in your area. <br />
            </strong>
            Use the <strong>Request support</strong> button and we will match
            you with a volunteer in your area!
          </p>
        </div>
      ) : (
        ""
