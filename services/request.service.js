  ...
  try {
    ...
    requests.forEach((request) => {
      request.status.volunteers.forEach((volunteer_obj) => {
        if (
          volunteer_obj.volunteer === id &&
          volunteer_obj.current_status === parseInt(status)
        ) {
          // Check that the volunteer status for the desired volunteer matches the requested status
          if (request.status.current_status === request_status.COMPLETED) {
            // Special case -> if a request is complete, only send it back to the volunteers who completed it
...
async function handleDirectMatch(request) {
  ...
  let volunteer_statuses = [volunteer_status_obj];
  ...
  if (request.association) {
    let association = await AssociationService.getAssociation({
...
async function handleGeneral(request) {
  ...
  if (request.association) {
    let association = (
      await AssociationService.getAssociation({ _id: request.association })
    )[0];
    associationEmail = association.email;
  ...
  pusher.trigger(
    request.association,
    "general",
    "You have a new unmatched request!"
  );
...
exports.matchVolunteers = async function (requestID, volunteers, adminMessage) {
  try {
    ...
    let new_volunteers = set_volunteers.map(function (volunteer_id) {
      if (
        !request.status.volunteers ||
        request.status.volunteers.length === 0 ||
        request.status.volunteers.filter(
          (volunteer) => volunteer.volunteer === volunteer_id
        ).length === 0
      ) {
        ...
        let volunteer = volunteer_copy.filter(
          (volunteer) => volunteer.volunteer === volunteer_id
        )[0];
        volunteers_to_be_notified.push(volunteer_id);
    ...
    updatedRequest.status.volunteers.forEach((volunteer) => {
      if (
        volunteer.current_status === volunteer_status.IN_PROGRESS ||
        volunteer.current_status === volunteer_status.PENDING
      ) {
        matchedVolunteers += 1;
      }
...
exports.unmatchVolunteers = async function (requestID, volunteers) {
  try {
    ...
    let removing_volunteers = !volunteers
      ? request.status.volunteers.map(function (volunteer) {
          ...
          if (
            request.status.volunteers.filter(
              (volunteer) =>
                ...
                volunteer.current_status !== volunteer_status.REJECTED
            ).length !== 0
          ) {
            // Get all attached volunteers from the volunteers list who have not already rejected the request
            return volunteer_id;
          }
        });
    ...
    if (removing_volunteers.length > 0) {
      ...
      current_request.status.volunteers.forEach(function (v, index, array) {
        if (
          removing_volunteers.filter(
            (volunteer_id) => volunteer_id === v.volunteer
          ).length !== 0
        ) {
...
exports.acceptRequest = async function (volunteerID, requestID) {
  try {
    ...
    await RequestRepository.updateRequestComplex(
      { _id: requestID, "status.volunteers.volunteer": volunteerID },
      {
        $set: {
 ...
 */
exports.completeRequest = async function (
  volunteerID,
  requestID,
  reason,
  volunteer_comment,
  ...
  try {
    ...
    allRequests.forEach((request) => {
      request.status.volunteers.forEach((volunteer) => {
        if (
          volunteer.current_status === volunteer_status.PENDING &&
          volunteer.last_notified_time <
            new Date(Date.now() - expiryTime * 60 * 60 * 1000)
        ) {
  ...
  try {
    ...
    allRequests.forEach((request) => {
      request.status.volunteers.forEach((volunteer) => {
        if (
          volunteer.current_status === volunteer_status.IN_PROGRESS &&
          volunteer.last_notified_time <
            new Date(Date.now() - notificationTime * 60 * 60 * 1000)
        ) {
    ...
    for (var request in reminderVolunteers) {
      ...
      for (var i = 0; i < users.length; i++) {
        ...
        await RequestRepository.updateRequestComplex(
          { _id: request, "status.volunteers.volunteer": user._id },
          {
            $set: {
              "status.volunteers.$.last_notified_time": new Date(),
            },
