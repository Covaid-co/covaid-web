...
exports.registerAdmin = async function (admin) {
  try {
    ...
    if (valid) {
      ...
      var new_admin = await AssociaitonAdminRepository.createAssociationAdmin(
        admin
      );
      var new_admin_name = new_admin.first_name + " " + new_admin.last_name;
      await Association.findByIdAndUpdate(admin.association_id, {
        $push: { admins: { email: admin.email, name: new_admin_name } },
      });
      return new_admin;
    } else {
...
exports.login_admin = async function (admin) {
  try {
    ...
    let found_admin = await AssociaitonAdminRepository.readAssociationAdmin({
      email: { $regex: new RegExp(email, "i") },
    });
    if (
      found_admin.length === 0 ||
      !validate_password(found_admin[0], admin.password)
    ) {
...
const validate_password = (admin, incomingPassword) => {
  ...
  crypto
    .pbkdf2Sync(incomingPassword, admin.password.salt, 10000, 512, "sha512")
    .toString("hex");
  return admin.password.hash === hash;
};

const validate_email_accessibility = async (email) => {
  let result = await AssociaitonAdminRepository.readAssociationAdmin({
    email: email,
  });
...
const getHash = (plainTextPassword, salt) => {
  return crypto
    .pbkdf2Sync(plainTextPassword, salt, 10000, 512, "sha512")
    .toString("hex");
};
